...
class PointEnv(mujoco_env.MujocoEnv, utils.EzPickle):
  ...
  def step(self, action):
    ...
    drag_len = 0.15*10
    qpos_temp[0] = qpos_temp[0] + dx * drag_len
    qpos_temp[1] = qpos_temp[1] + dy * drag_len

    pos_temp_after = deepcopy(qpos_temp[:2])

    if np.linalg.norm(pos_temp_after - pos_temp_before) > drag_len:
      # Find direction
    ...
    if np.linalg.norm(pos_after) - np.linalg.norm(pos_before) > 1.2:
      reward = 0
    else:
      reward = -1
