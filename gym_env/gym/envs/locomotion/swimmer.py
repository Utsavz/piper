...
from gym.envs import offline_env
...
class SwimmerEnv(mujoco_env.MujocoEnv, utils.EzPickle):
  ...
  def _get_obs(self):
    if self._expose_all_qpos:
      obs = np.concatenate([
          self.physics.data.qpos.flat[:5].copy(),  # Ensures only swimmer obs.
          self.physics.data.qvel.flat[:5].copy(),
      ])
    else:
      obs = np.concatenate([
