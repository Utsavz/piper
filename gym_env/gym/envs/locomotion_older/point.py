...
class PointEnv(mujoco_env.MujocoEnv, utils.EzPickle):
  ...
  def step(self, action):
    drag_len = 0.6
    action[0] = drag_len * action[0]
    action[1] = drag_len * action[1]
    ...
    current_position = deepcopy(qpos[:2])
    reward = 0
    done = False
    info = {'position':deepcopy(current_position)}
  ...
  def _reset_model(self):
    ...
    qpos[:2] = ([0,0])#self._rowcol_to_xy([0,0])
    self.set_state(qpos, qvel)
